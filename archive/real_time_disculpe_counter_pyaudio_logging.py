    ...
    while True:
        try:
            ...
            if len(buffer) >= model.sample_rate * target_duration:
                ...
                # Transcribe
                segments, info = model.transcribe(chunk, language='es', beam_size=5)
...
if __name__ == "__main__":
    ...
    try:
        model = WhisperModel("small", device="cuda", compute_type="float16")  # Adjust parameters as needed
...
from faster_whisper import (
    WhisperModel,
)  # Ensure faster-whisper is installed and compatible
...
def get_device_info_by_name(target_name):
    ...
    try:
        for device in list_audio_devices():
            if device["name"] == target_name:
                logging.debug(
                    f"Found target device '{target_name}' with index {device['index']}."
                )
                return (
                    device["index"],
...
if __name__ == "__main__":
    ...
    # Specify the device name you want to record from
    desired_device_name = (
        "CABLE Output (VB-Audio Virtual Cable)"  # Updated to use CABLE Output
    )

    # Get device index, max input channels, and default sample rate
    device_index, max_input_channels, default_sample_rate = get_device_info_by_name(
        desired_device_name
    )
    if device_index is None:
        logging.error(
            f"Exiting script as device '{desired_device_name}' was not found."
        )
        sys.exit(1)
    ...
    if max_input_channels >= 1:
        ...
        logging.error(
            f"Device '{desired_device_name}' does not support input channels."
        )
        sys.exit(1)
    RATE = default_sample_rate  # Use device's default sample rate
    CHUNK = 1024  # Buffer size
    ...
    logging.debug(
        f"Stream parameters set: FORMAT={FORMAT}, CHANNELS={CHANNELS}, RATE={RATE}, CHUNK={CHUNK}"
    )
    ...
    try:
        ...
        model.sample_rate = (
            RATE  # Set the sample rate attribute for buffer calculations
        )
        logging.info("Whisper model loaded successfully.")
    except Exception as e:
        logging.exception(f"Failed to load Whisper model: {e}")
    ...
    try:
        ...
        logging.info(
            f"Recording from '{desired_device_name}' with {CHANNELS} channel(s) at {RATE} Hz..."
        )
        stream.start_stream()
